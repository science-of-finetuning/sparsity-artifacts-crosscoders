...
def test_max_activation_computation(activation_test_params):
    ...
    expected_max = th.tensor([
        num_samples - 1 + num_tokens - 1 + j + base_value 
    ...
    # Verify results
    assert th.allclose(max_acts_latent, expected_max), \
    ...
    with patch('torch.utils.data.DataLoader', autospec=True) as mock_dataloader:
        ...
        # Compute max activations
        max_acts_paired = compute_max_activation(crosscoder, cache, device, batch_size, num_workers=0)
        
        # Verify results
        assert th.allclose(max_acts_paired, expected_max), \
