...
df.loc[latent_ids[merged_df["feature"].astype(int)], "autointerp_acc_fuzzing"] = merged_df["accuracy_fuzzing"].tolist()
df.loc[latent_ids[merged_df["feature"].astype(int)], "autointerp_acc_detection"] = merged_df["accuracy_detection"].tolist()
...
chat_only_df['beta_ratio_reconstruction_rank'] = chat_only_df['beta_ratio_reconstruction'].rank()

shared_df['beta_ratio_error_rank'] = shared_df['beta_ratio_error'].rank()
shared_df['beta_ratio_reconstruction_rank'] = shared_df['beta_ratio_reconstruction'].rank()

# Print some statistics about the ranks
print(f"Beta ratio error rank range: {chat_only_df['beta_ratio_error_rank'].min()} to {chat_only_df['beta_ratio_error_rank'].max()}")
print(f"Beta ratio reconstruction rank range: {chat_only_df['beta_ratio_reconstruction_rank'].min()} to {chat_only_df['beta_ratio_reconstruction_rank'].max()}")


# %%
# Create scatter plot with color based on beta_ratio_error
plt.figure(figsize=(10, 8))
scatter = plt.scatter(chat_only_df['autointerp_acc_detection'], 
                     chat_only_df['autointerp_acc_fuzzing'],
                     alpha=0.6,
                     c=chat_only_df['beta_ratio_error_rank'] + chat_only_df['beta_ratio_reconstruction_rank'],
                     cmap='viridis')

# Add diagonal line
min_val = min(chat_only_df['autointerp_acc_detection'].min(), chat_only_df['autointerp_acc_fuzzing'].min())
max_val = max(chat_only_df['autointerp_acc_detection'].max(), chat_only_df['autointerp_acc_fuzzing'].max())
...
def compare_percentiles(df, percentile=10):
    ...
    valid_df = df.dropna(subset=['beta_ratio_error_rank', 'beta_ratio_reconstruction_rank', 
                                'autointerp_acc_detection', 'autointerp_acc_fuzzing'])
    
    if len(valid_df) < len(df):
        print(f"Removed {len(df) - len(valid_df)} rows with NaN values")
    
    # Calculate the sum of ranks
    sum_ranks = valid_df['beta_ratio_error_rank'] + valid_df['beta_ratio_reconstruction_rank']
    
    # Calculate percentile thresholds
    bottom_threshold = np.percentile(sum_ranks, percentile)
    top_threshold = np.percentile(sum_ranks, 100 - percentile)
    print(f"Bottom threshold: {bottom_threshold:.2f}, Top threshold: {top_threshold:.2f}")
    ...
    print(f"Bottom percentile size: {len(bottom_percentile_df)}, Top percentile size: {len(top_percentile_df)}")
    
    # Print autointerp scores for each group
    print(f"\nAutointerp scores for features in bottom {percentile}% (better beta ratios):")
    print(f"Detection accuracy: {bottom_percentile_df['autointerp_acc_detection'].mean():.4f} ± {bottom_percentile_df['autointerp_acc_detection'].std():.4f}")
    print(f"Fuzzing accuracy: {bottom_percentile_df['autointerp_acc_fuzzing'].mean():.4f} ± {bottom_percentile_df['autointerp_acc_fuzzing'].std():.4f}")
    
    print(f"\nAutointerp scores for features in top {percentile}% (worse beta ratios):")
    print(f"Detection accuracy: {top_percentile_df['autointerp_acc_detection'].mean():.4f} ± {top_percentile_df['autointerp_acc_detection'].std():.4f}")
    print(f"Fuzzing accuracy: {top_percentile_df['autointerp_acc_fuzzing'].mean():.4f} ± {top_percentile_df['autointerp_acc_fuzzing'].std():.4f}")
    
    # Perform t-test to check if the differences are statistically significant
    from scipy import stats
    
    detection_ttest = stats.ttest_ind(bottom_percentile_df['autointerp_acc_detection'], 
                                      top_percentile_df['autointerp_acc_detection'], 
                                      equal_var=False)
    fuzzing_ttest = stats.ttest_ind(bottom_percentile_df['autointerp_acc_fuzzing'], 
...
valid_df = chat_only_df.dropna(subset=['beta_ratio_error_rank', 'beta_ratio_reconstruction_rank', 
                            'autointerp_acc_detection', 'autointerp_acc_fuzzing'])
...
p = np.poly1d(z)
plt.plot(
    sorted(valid_df['beta_ratio_error_rank'] + valid_df['beta_ratio_reconstruction_rank']),
    p(sorted(valid_df['beta_ratio_error_rank'] + valid_df['beta_ratio_reconstruction_rank'])),
    "r--",
    alpha=0.8
)
...
plt.title(f'Detection Accuracy vs. Sum of Beta Ratio Ranks\nCorrelation: {corr:.3f} (p={p_value:.4f})')
...
valid_df['rank_sum'] = valid_df['beta_ratio_error_rank'] + valid_df['beta_ratio_reconstruction_rank']
...
valid_df['percentile_bin'] = pd.cut(valid_df['rank_sum'], bins=percentiles, labels=bin_labels)

# Group by percentile bin and calculate statistics
grouped = valid_df.groupby('percentile_bin')['autointerp_acc_detection'].agg(['mean', 'std', 'count'])
...
dec_percentiles = np.percentile(valid_df['dec_norm_diff_rank'], np.arange(0, 101, percentile_range))
valid_df['dec_percentile_bin'] = pd.cut(valid_df['dec_norm_diff_rank'], bins=dec_percentiles, labels=bin_labels)

# Group by dec_norm_diff percentile bin and calculate statistics
dec_grouped = valid_df.groupby('dec_percentile_bin')['autointerp_acc_detection'].agg(['mean', 'std', 'count'])
...
freq_percentiles = np.percentile(valid_df['freq_rank'], np.arange(0, 101, percentile_range))
valid_df['freq_percentile_bin'] = pd.cut(valid_df['freq_rank'], bins=freq_percentiles, labels=bin_labels)

# Group by freq percentile bin and calculate statistics
freq_grouped = valid_df.groupby('freq_percentile_bin')['autointerp_acc_detection'].agg(['mean', 'std', 'count'])
freq_grouped['se'] = freq_grouped['std'] / np.sqrt(freq_grouped['count'])  # Standard error
...
plt.ylabel('Average Detection Accuracy')
plt.title('Detection Accuracy by Cumulative Percentile of Ranks\nwith 95% Confidence Intervals')
plt.legend()
...
# %%
valid_df.plot.scatter(x="rank_sum", y="dec_norm_diff_rank", c="autointerp_acc_detection", cmap="viridis")
# %%
...
def plot_percentile_comparison(valid_df, percentile_range=10, metric='autointerp_acc_detection', 
                              figsize=(10, 6), include_metrics=None):
    """
    ...
    if 'dec_norm_diff_rank' not in valid_df.columns and 'dec_norm_diff' in include_metrics:
        valid_df = valid_df.copy()
        valid_df['dec_norm_diff_rank'] = valid_df['dec_norm_diff'].rank(ascending=True)
    ...
    metric_styles = {
        'beta_ratio': {'color': 'blue', 'marker': 'o', 'linestyle': '-', 
                       'label': 'Beta Ratio Ranks', 'rank_col': 'rank_sum'},
        'dec_norm_diff': {'color': 'red', 'marker': 's', 'linestyle': '--', 
                          'label': 'Dec Norm Diff Ranks', 'rank_col': 'dec_norm_diff_rank'},
        'freq': {'color': 'green', 'marker': '^', 'linestyle': '-.', 
                 'label': 'Frequency Ranks', 'rank_col': 'freq_rank'}
    }
    ...
    for metric_name in include_metrics:
        ...
        percentiles = np.percentile(valid_df[rank_col], np.arange(0, 101, percentile_range))
        bin_col = f'{metric_name}_percentile_bin'
        valid_df[bin_col] = pd.cut(valid_df[rank_col], bins=percentiles, labels=bin_labels)
...
fig = plot_percentile_comparison(valid_df, percentile_range=20, include_metrics=["beta_ratio"], figsize=(8, 4))
...
spearman_corr, p_value = stats.spearmanr(valid_df['beta_ratio_reconstruction'], valid_df['freq'])
print(spearman_corr, p_value)
spearman_corr, p_value = stats.spearmanr(valid_df['beta_ratio_error'], valid_df['freq'])
print(spearman_corr, p_value)
# %%
valid_df.plot.scatter(x="beta_ratio_reconstruction", y="beta_ratio_error", c=np.log10(valid_df["freq"]), colorbar=True, cmap="viridis", alpha=0.5)
# %%
