...
if __name__ == "__main__":
    ...
    run_name = f"S{args.seed}-L{args.layer}-mu{args.mu:.1e}-lr{args.lr:.0e}" + (f"-{args.run_name}" if args.run_name is not None else "") + ("-ZeroInit" if args.zero_init_scaler else "")
    if args.random_indices:
