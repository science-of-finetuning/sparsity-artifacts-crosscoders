...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cc", type=str, default="checkpoints/l13-mu4.0e-02-lr1e-04/ae_90000.pt"
    )
    ...
    batch_size = 16
    subset = 10000
    stats_fineweb = feature_statistics(
        validation_set_base.select_columns("text").select(range(subset)),
        tokenizer,
        base_model,
        instruction_model,
        cc,
    ...
    stats_lmsys = feature_statistics(
        validation_set_chat.select_columns("text").select(range(subset)),
        tokenizer,
        base_model,
        instruction_model,
        cc,
