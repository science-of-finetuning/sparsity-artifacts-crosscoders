...
def main():
    ...
    if len(shared_indices) > args.shared_sample_size:
        shared_sample = th.tensor(
            np.random.choice(
                shared_indices, size=args.shared_sample_size, replace=False
            )
        )
    else:
        logger.warning(
            f"Requested sample size {args.shared_sample_size} is larger than available shared indices ({len(shared_indices)})"
        )
    ...
    th.save(
        low_norm_diff_indices_tensor,
        results_dir / f"low_norm_diff_indices_{args.low_norm_diff_sample_size}.pt",
    )
