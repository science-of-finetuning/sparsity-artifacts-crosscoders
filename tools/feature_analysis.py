...
from .feature_utils import filter_dead_features, mask_to_indices, remove_dead_and_filter, dead_feature_indices
...
def get_features(batch, base_model, instruction_model, ae, layer):
    ...
    rescaled_features_split = features_split * ae.decoder.weight.norm(dim=2).unsqueeze(0)
    rescaled_features_joint = features_joint * ae.decoder.weight.norm(dim=2).sum(dim=0, keepdim=True)

    base_features = features_split.view(batch_size, -1, features_split.shape[-2], features_split.shape[-1])[..., 0, :]
    instruction_features = features_split.view(batch_size, -1, features_split.shape[-2], features_split.shape[-1])[..., 1, :]

    base_features_rescaled = rescaled_features_split.view(batch_size, -1, rescaled_features_split.shape[-2], rescaled_features_split.shape[-1])[..., 0, :]
    instruction_features_rescaled = rescaled_features_split.view(batch_size, -1, rescaled_features_split.shape[-2], rescaled_features_split.shape[-1])[..., 1, :]

    return Features(base=base_features_rescaled, instruction=instruction_features_rescaled, joint=rescaled_features_joint), Features(base=base_features, instruction=instruction_features, joint=features_joint)
...
def filter_stack_features(features, attention_mask):
    ...
    joint_features = features.joint.view(-1, features.joint.shape[-1])[attention_mask.view(-1).bool()]
...
def compute_statistics(features, total_tokens, non_zero_threshold=1e-8):
    ...
    base_statistic = FeatureStatistic(base_avg_activation, base_non_zero_counts, total_tokens)
    instruction_statistic = FeatureStatistic(instruction_avg_activation, instruction_non_zero_counts, total_tokens)
    joint_statistic = FeatureStatistic(joint_avg_activation, joint_non_zero_counts, total_tokens)
...
def plot_feature_diff(save_dir, stats_fineweb, stats_lmsys, group_name=None, rescaled=True, indices_path=None, save=True):
    ...
    lmsys_hist = go.Histogram(x=lmsys_filtered_diff.cpu().numpy(), nbinsx=50, name="LMSYS", marker_color=lmsys_color)
