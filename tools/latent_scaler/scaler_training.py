...
class FeatureScaler(th.nn.Module):
    def __init__(
        self,
        dict_size: int,
        fixed_mask: th.Tensor | None = None,
        ...
        if fixed_mask is None:
            self.scaler = th.nn.Parameter(
                self.get_init_vector(dict_size, use_elu, zero_init)
            )
            self.fixed_mask = None
        else:
            self.register_buffer("fixed_mask", fixed_mask)
            self.scaler = th.nn.Parameter(
                self.get_init_vector((~fixed_mask).sum(), use_elu, zero_init)
            )
    ...
    def get_init_vector(
        ...
        if use_elu:
            return (
                th.zeros(size, device=device)
                if not init_zeros
                else th.ones(size, device=device) * -10
            )
        else:
            return (
                th.ones(size, device=device)
                if not init_zeros
                else th.zeros(size, device=device)
            )
...
class FeatureScalerTrainer(CrossCoderTrainer):
    def __init__(
        self,
        cross_coder: CrossCoder,
        target_decoder_layers: list[int] = None,
        feature_scaler: FeatureScaler | None = None,
        **kwargs
    ):
        assert (
            "pretrained_ae" not in kwargs
        ), "pretrained_ae should not be set for FeatureScalerTrainer"
        assert "layer" not in kwargs, "layer should not be set for FeatureScalerTrainer"
        assert (
            "lm_name" not in kwargs
        ), "lm_name should not be set for FeatureScalerTrainer"
        assert (
            "submodule_name" not in kwargs
        ), "submodule_name should not be set for FeatureScalerTrainer"
        assert (
            "resample_steps" not in kwargs
        ), "resample_steps should not be set for FeatureScalerTrainer"
        self.compile = kwargs.pop("compile", False)
        super().__init__(
            **kwargs,
            pretrained_ae=cross_coder,
            layer=-1,
            lm_name="feature_scaler",
            compile=False
        )
        ...
        self.ae.decoder.weight = nn.Parameter(
            cross_coder.decoder.weight.data[target_decoder_layers, :, :]
        )
        self.ae.decoder.bias = nn.Parameter(
            cross_coder.decoder.bias.data[target_decoder_layers, :]
        )
    ...
    def loss(self, x, logging=False, return_deads=False, **kwargs):
        ...
        l2_loss = th.linalg.norm(
            x[:, self.target_decoder_layers] - x_hat, dim=-1
        ).mean()
...
class IndividualFeatureScalerTrainer(CrossCoderTrainer):
    def __init__(
        ...
        target_decoder_layer: int,
        source_decoder_weights: th.Tensor,
        zero_init: bool = False,
        dtype=th.float64,
        **kwargs
    ):
        ...
        assert self.source_decoder_weights.shape == (
            len(feature_indices),
            cross_coder.activation_dim,
        )
        ...
        self.ae.decoder.weight = nn.Parameter(
            cross_coder.decoder.weight.data[[target_decoder_layer], :, :].to(dtype)
        )
        self.ae.decoder.bias = nn.Parameter(
            cross_coder.decoder.bias.data[[target_decoder_layer], :].to(dtype)
        )
        ...
        if dtype == th.float64:
            ...
            with th.no_grad():
                ...
                assert self.ae.decoder.weight.shape == (
                    1,
                    self.ae.dict_size,
                ...
                assert th.all(
                    self.ae.decoder.weight[0, :, :]
                    == cross_coder.decoder.weight[target_decoder_layer, :, :]
                )
                assert th.all(
                    self.ae.decoder.bias[0, :]
                    == cross_coder.decoder.bias[target_decoder_layer, :]
                )
                test_input = th.randn(
                    10, 2, cross_coder.activation_dim, device=self.device, dtype=dtype
                )  # (batch_size, num_layers, activation_dim)
            ...
            logger.warning(
                "Skipping correctness tests for float32. Set dtype to float64 to run them."
            )
        ...
        self.feature_mask = th.zeros(
            (cross_coder.dict_size), dtype=bool, device=self.device
        )
        ...
        if self.compile:
            raise NotImplementedError(
                "FeatureScalerTrainer does not support compilation"
            )
    ...
    def test_correctness(self):
        ...
        activations = th.randn(
            100, 2, self.ae.activation_dim, dtype=self._dtype, device=self.device
        )  # (batch_size, num_layers, activation_dim)
        ...
        modified_crosscoder.encoder.weight = nn.Parameter(
            self.ae.encoder.weight.data.to(self._dtype)
        )
        modified_crosscoder.encoder.bias = nn.Parameter(
            self.ae.encoder.bias.data.to(self._dtype)
        )
        modified_crosscoder.decoder.bias = nn.Parameter(
            self.ae.decoder.bias.data.to(self._dtype)
        )
        modified_decoder_weights = th.clone(self.ae.decoder.weight.data.to(self._dtype))
        modified_decoder_weights[0, toy_feature_mask, :] = self.source_decoder_weights[
            [0]
        ].to(self._dtype)
        modified_crosscoder.decoder.weight = nn.Parameter(
            modified_decoder_weights.to(self._dtype)
        )
    ...
    def forward(self, activations, return_features=False, scale_features=True):
        ...
        target_reconstruction = self.ae.decode(f)[
            :, 0
        ]  # The crosscoder only has a single decoder layer -> we only need the first layer
        ...
        assert target_decoder_target.shape == (
        ...
        assert target_decoder_target.shape == (
            batch_size * len(self.feature_indices),
            activation_dim,
        )
        ...
        assert target_reconstruction_target.shape == (
            batch_size * len(self.feature_indices),
            activation_dim,
        )
        ...
        assert source_decoder_target.shape == (
            len(self.feature_indices),
            activation_dim,
        )
        ...
        assert source_reconstruction_target.shape == (
            batch_size * len(self.feature_indices),
            activation_dim,
        )
        ...
        target_reconstruction_interleaved = target_reconstruction.repeat_interleave(
            len(self.feature_indices), dim=0
        )
        assert target_reconstruction_interleaved.shape == (
            batch_size * len(self.feature_indices),
            activation_dim,
        )
        x_hat = (
            target_reconstruction_interleaved
            - target_reconstruction_target
            + source_reconstruction_target
        )
    ...
    def loss(self, x, x_hat=None, logging=False, **kwargs):
        ...
        x = x[:, self.target_decoder_layer].repeat_interleave(
            len(self.feature_indices), dim=0
        )
        ...
        x_hat_reshaped = x_hat.view(
            batch_size, len(self.feature_indices), activation_dim
        )
        ...
        residual_variance = th.var(x_reshaped - x_hat_reshaped, dim=0).sum(
            dim=1
        )  # [num_features]
