...
def dead_feature_indices(combined_feature_statistics, rescaled=False):
    if rescaled:
        mask = th.stack([statistic.rescaled.joint.non_zero_counts > 0 for statistic in combined_feature_statistics]).all(dim=0)
    else:
        mask = th.stack([statistic.normal.joint.non_zero_counts > 0 for statistic in combined_feature_statistics]).all(dim=0)
