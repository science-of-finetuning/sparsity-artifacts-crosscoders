...
def plot_decoder_norm_diff(
    ...
    plt.hist(
        values[
            (
                (values >= 0.9)
                & (ratio_error_values < thres_error)
                & (ratio_reconstruction_values < thres_reconstruction)
            )
        ],
        bins=bins,
        ...
        + str(thres_error)
        + r" $ \& $\nu^r < "
        + str(thres_reconstruction)
        + r" $}",
    )
    plt.hist(
        values[(values <= 0.1)], bins=bins, color=green, label="base-only", log=log
    )
...
plt.scatter(df_cc["dec_norm_diff"], df_cc["rank_sum"], alpha=0.5, label="L1")
...
corr_error, p_error = scipy.stats.pearsonr(
    cc_error["dec_norm_diff"], cc_error["beta_ratio_error"]
)
corr_recon, p_recon = scipy.stats.pearsonr(
    cc_recon["dec_norm_diff"], cc_recon["beta_ratio_reconstruction"]
)
...
corr_error, p_error = scipy.stats.pearsonr(
    topk_error["dec_norm_diff"], topk_error["beta_ratio_error"]
)
corr_recon, p_recon = scipy.stats.pearsonr(
    topk_recon["dec_norm_diff"], topk_recon["beta_ratio_reconstruction"]
)
